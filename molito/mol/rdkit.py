...
class RDKitMol(MolRepr):
    ...
    def _from_lazy(cls, payload: LazyData, meta: Mapping[str, Any]) -> Self:
        """Build a wrapper without decoding RDKit. Subclasses with extra state can override this factory."""
    ...
    def rdkit_mol(self) -> Chem.rdchem.Mol:
        """The owned mutable molecule, decoded once on first access when loaded from HDF5."""

        if isinstance(self._mol, LazyData):
            self._mol = Chem.Mol(self._mol.read().tobytes())
    ...
    def n_heavy_atoms(self) -> int:
        return self.rdkit_mol.GetNumHeavyAtoms()
    ...
    def n_conformers(self) -> int:
        return self.rdkit_mol.GetNumConformers()
    ...
    def coords(self) -> TArr | None:
        ...
        return np.stack([conf.GetPositions() for conf in self.rdkit_mol.GetConformers()])

    def copy(self) -> Self:
        return type(self)(self.rdkit_mol, meta=copy.deepcopy(dict(self.meta)))
    ...
    def to_bytes(self) -> bytes:
        ...
        payload = base64.b64encode(self.rdkit_mol.ToBinary(flags)).decode("ascii")
        obj = {
            "version": 1,
            "kind": "RDKitMol",
